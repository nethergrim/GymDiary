...
public class AddingMeasurementActivity extends BasicMenuActivity {
	...
	public void onClick(View arg0) {
		...
		if (id == R.id.btnAddingMeasurementsToDB) {
			...
			if (!weight_m.isEmpty()) {
				db.addRec_Measure(date,
						getResources().getString(R.string.weight), weight_m);
				areEmpty = false;
			}
			if (!tall_m.isEmpty()) {
				db.addRec_Measure(date,
						getResources().getString(R.string.tall), tall_m);
				areEmpty = false;
			}
			if (!chest_m.isEmpty()) {
				db.addRec_Measure(date, getResources()
						.getString(R.string.chest), chest_m);
				areEmpty = false;
			}
			if (!waist_m.isEmpty()) {
				db.addRec_Measure(date, getResources()
						.getString(R.string.waist), waist_m);
				areEmpty = false;
			}
			if (!hip_m.isEmpty()) {
			...
			if (!calf_m.isEmpty()) {
				db.addRec_Measure(date,
						getResources().getString(R.string.calf), calf_m);
				areEmpty = false;
			}
			if (!arm_m.isEmpty()) {
