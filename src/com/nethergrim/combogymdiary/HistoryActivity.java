...
public class HistoryActivity extends BasicMenuActivity  implements LoaderCallbacks<Cursor>{
	...
	public boolean onContextItemSelected(MenuItem item) {
	    ...
	    if (item.getItemId() == CM_DELETE_ID) {	      
	    	...
	    	int id  =(int) acmi.id;
	    	String[] args = {""+id};
	    	Cursor c = db.getDataMain(null, DB.COLUMN_ID+"=?", args, null, null, null);
	    	c.moveToFirst();
	    	String dateToDelete = c.getString(3);
	    	Log.d(LOG_TAG, "position to delete == "+pos+" id == "+id+" _id == "+c.getInt(0)+" date == "+dateToDelete);
	    	String[] argsDate = {dateToDelete};
	    	Cursor cur = db.getDataMain(null, DB.DATE+"=?", argsDate, null, null, null);
	    	Log.d(LOG_TAG, "Goint to delete "+cur.getCount()+" exercises from history");
	    	if (cur.moveToFirst()){
	    		do{
